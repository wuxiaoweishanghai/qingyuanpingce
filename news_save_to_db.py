...
def save_to_dbsuccess(line):
    ...
    """
    global save_to_db
    pattern = re.compile('.*Successfully.*write the news into news db.*')
...
def main():
    ...
    logging.basicConfig(level=logging.INFO,
                ...
                filename='newssavetodb.log',
                filemode='w')
    operatedb = OperateDB("./mysql.conf")
    operatedb.connect()
    pool = redis.ConnectionPool(host='localhost', port=6385, db=0)
    ...
    while True:
        ...
        if len(filelist) == 0:
            ...
            if now >= begin_time and now <= end_time:
                cmd = "sh -x ./clear_savenewsdb.sh"
                os.system(cmd)
                sys.exit()
            else:
                now.strftime('%Y-%m-%d 00:00:00')
                now = str(now).split(' ')[0] + "%"
                sql = "select time from News_Flow_Funnel where Stage = 400 and time like \'%s\' order by time desc"%(now)
