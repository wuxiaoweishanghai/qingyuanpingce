...
from evaluting.Videoofflinereason import Video_Offline_Reason
reload(sys)
sys.setdefaultencoding("utf-8")
setup_logging()

class Video_offlinereason(object):
    """
    video database
    """
    def __init__(self):
        ...
        init
        """
        self.logger = logging.getLogger("Video_Offline_reason")
    ...
    def get_result(self, now):
        ...
        """
        sql = "select count(*) as content_num, off_reason_type from video_info\
                where check_in_time >=unix_timestamp(\'%s\') and review_status = 6 group by off_reason_type"%(now)
        print sql
        try:
    ...
    def is_exist(self, now, OfflineReason):
        ...
        is_exist
        """
        sql = "select * from Video_Offline_Info where \
    ...
    def update_db(self, now, results):
        ...
        #for result in otherreason_results:
            ...
            new_record = Video_Offline_Reason(
            ...
            try:
                with session_evaluting() as session:
                    if self.is_exist(now, OfflineReason):
                        query = session.query(Video_Offline_Reason)
                        query.filter(Video_Offline_Reason.OfflineReason == OfflineReason, Video_Offline_Reason.time == update_time).\
                                update(update_dict)
                    else:
...
if __name__ == '__main__':
    video_offline_reason = Video_offlinereason()
    video_offline_reason.run()
