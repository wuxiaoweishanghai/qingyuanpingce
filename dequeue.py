...
def main():
    ...
    logging.basicConfig(level=logging.INFO,
                ...
                filename='dequeue.log',
                filemode='w')
    operatedb = OperateDB("./mysql.conf")
    operatedb.connect()
    pool = redis.ConnectionPool(host='localhost', port=6382, db=0)
    ...
    while True:
        ...
        if len(filelist) == 0:
            ...
            if now >= begin_time and now <= end_time:
                cmd = 'sh -x ./clear_mq.sh'
                os.system(cmd)
                sys.exit()
            else:
                now.strftime('%Y-%m-%d 00:00:00')
                now = str(now).split(' ')[0] + "%"
                sql_cmd = "select time from Image_Flow_Funnel where Stage = 300 and time like \'%s\' order by time desc"%(now)
