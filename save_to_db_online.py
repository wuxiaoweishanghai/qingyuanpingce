...
def handle_file(filename):
    ...
    with open(filename, 'r') as fp:
        if index ==0:
            line = fp.readline()
            while len(line) != 0:
                save_to_db = save_to_dbonline(line)
                count = count + 1
                line = fp.readline()
        else:
            while index != 0:
...
def save_to_dbonline(line):
    """
    保存入库成功的数据和对应的类目id
    """
    global save_to_db
    pattern = re.compile('.*save to db success.*status:3.*')
...
def main():
    ...
    logging.basicConfig(level=logging.INFO,
                ...
                filename='savetodbonline.log',
                filemode='w')
    operatedb = OperateDB("./mysql.conf")
    operatedb.connect()
    pool = redis.ConnectionPool(host='localhost', port=6383, db=0)
    ...
    while True:
        ...
        if len(filelist) == 0:
            ...
            if now >= begin_time and now <= end_time:
                ...
                now = str(now).split(' ')[0] + "%"
                sql = "select time from Image_Flow_Funnel where Stage = 500 and time like \'%s\' order by time desc"%(now)
