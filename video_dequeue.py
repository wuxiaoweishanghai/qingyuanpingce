...
def get_dequeue_flow(line):
    ...
    """
    global dequeue
    pattern = re.compile('.*into map, the sequence is.*')
...
def main():
    ...
    logging.basicConfig(level=logging.INFO,
                ...
                filename='videodequeue.log',
                filemode='w')
    operatedb = OperateDB("./mysql.conf")
    operatedb.connect()
    pool = redis.ConnectionPool(host='localhost', port=6388, db=0)
    ...
    while True:
        ...
        if len(filelist) == 0:
            ...
            if now >= begin_time and now <= end_time:
                cmd = 'sh -x ./clear_videomq.sh'
                os.system(cmd)
                sys.exit()
            else:
                now.strftime('%Y-%m-%d 00:00:00')
                now = str(now).split(' ')[0] + "%"
                sql_cmd = "select time from Video_Flow_Funnel where Stage = 300 and time like \'%s\' order by time desc"%(now)
